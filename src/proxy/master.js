...
class ProxyMaster {
  ...
  async getProxies(req, res) {
    log.info(`MASTER - Fetching request proxies. Currently available ${this.proxies}`)
    res.json(this.proxies.map(p => p.host))
  }

  async handleProxyConn(client) {
    log.info(`MASTER - Received proxy connection from ${client}`)
    ...
    log.info(`MASTER - The first message is assigning host ${firstMessage.host}`)
    ...
    log.info(`MASTER - Proxy with host ${firstMessage.host} disconnected. Removing.`)
  ...
  async run() {
    ...
    log.info(`MASTER - Running http server at port ${config.PUBLIC_PORT}`)
    await new Promise((resolve, reject) => {
    ...
    log.info(`MASTER - Running socket io proxies server at port ${config.PRIVATE_HOST}:${config.PUBLIC_PORT}`)
    await new Promise((resolve, reject) => {
